...
export class ManagerDashboardComponent implements OnInit {
  ...
  puedeAprobarOperacion = computed(() =>
    (monto: number, moneda: string) => this.gestorService.puedeAprobarOperacion(monto, moneda)
  );

  getMensajeLimiteExcedido = computed(() =>
    (monto: number, moneda: string) => this.gestorService.getMensajeLimiteExcedido(monto, moneda)
  );
  ...
  approveOperation(id: number): void {
    ...
    if (!this.gestorService.puedeAprobarOperacion(operation.monto, operation.moneda)) {
      const message = this.gestorService.getMensajeLimiteExcedido(operation.monto, operation.moneda);
